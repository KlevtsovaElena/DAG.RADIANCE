...
if (isset($_POST['id'])) {
    $sqlText = "UPDATE `guest-houses` SET `name`='$name', `short-desc`='$shortDesc', `location`='$location', `link`='$link',  `image`='$imgTitle' WHERE `id`=$id";
    
    //запись в базу
    $query = $pdo->query($sqlText);
} else {
    $sqlText = "INSERT INTO `guest-houses` (`name`, `short-desc`, `location`, `link`, `image`) VALUES('$name', '$shortDesc', '$location', '$link', '$imgTitle')";
    
    // запись в базу
    $query = $pdo->query($sqlText);
    $sqlText = "SELECT id from `guest-houses` ORDER BY id DESC LIMIT 1 ;";
...
header('Location: ../card-house.php?id=' . $id)
